@section('title', 'Atur Hak Akses Peran')
@include('layouts.header')
@include('layouts.sidebar')

<main id="main" class="main">
    <div class="pagetitle">
      <h1>Atur Hak Akses: {{ ucfirst($role->name) }}</h1>
      <nav>
        <ol class="breadcrumb">
        ...
        <form action="{{ route('roles.update', $role->id) }}" method="POST">
                ...
                <div class="card-body pt-3">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $role->name) }}" required maxlength="50" {{ in_array(strtolower($role->name), ['admin', 'bendahara', 'sekretaris']) ? 'readonly' : '' }}>
                            @if(in_array(strtolower($role->name), ['admin', 'bendahara', 'sekretaris']))
                                <small class="text-muted"><i class="bi bi-info-circle me-1"></i>Nama peran sistem bawaan tidak dapat diganti.</small>
                            @else
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div>
                    ...
                    <small class="text-muted">Centang izin yang diberikan kepada pengguna dengan peran ini.</small>
                </div>
                            ...
                            <div class="card-body p-3">
                                <div class="row g-2">
                                    @foreach($perms as $permKey => $permLabel)
                                        @php
                                            $isChecked = in_array($permKey, $rolePermissions);
                                        @endphp
                                        <div class="col-md-6 col-12">
                                            <div class="form-check form-switch p-2 border rounded bg-white h-100 d-flex align-items-center">
                                                <input class="form-check-input ms-0 me-2 perm-checkbox" type="checkbox" name="permissions[]" value="{{ $permKey }}" id="perm_{{ $permKey }}" {{ $isChecked ? 'checked' : '' }}>
