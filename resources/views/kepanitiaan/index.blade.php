            ...
            <br>
                    ...
                    <tbody>
                        @foreach($kepanitiaans as $kepanitiaan)
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editKepanitiaanModal{{ $kepanitiaan->id_panitia }}">Edit</button>
                                    <form action="{{ route('kepanitiaan.destroy', $kepanitiaan->id_panitia ) }}" method="POST" style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
                            ...
                            <!-- Modal Edit -->
                            <div class="modal fade" id="editKepanitiaanModal{{ $kepanitiaan->id_panitia }}" tabindex="-1" aria-labelledby="editKepanitiaanModalLabel{{ $kepanitiaan->id_panitia }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="editKepanitiaanModalLabel{{ $kepanitiaan->id_panitia }}">Edit Posisi</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form action="{{ route('kepanitiaan.update', ['kepanitiaan' => $kepanitiaan->id_panitia]) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <div class="form-group">
                                                    ...
                                                    <select name="kegiatan_id_kegiatan" class="form-control" required>
                                                        @foreach($kegiatans as $kegiatan)
                                                            <option value="{{ $kegiatan->id_kegiatan }}" {{ $kepanitiaan->kegiatan_id_kegiatan == $kegiatan->id_kegiatan ? 'selected' : '' }}>{{ $kegiatan->nama_kegiatan }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="form-group">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select name="posisi_id_posisi" class="form-control" required>
                                                        @foreach($posisis as $posisi)
                                                            <option value="{{ $posisi->id_posisi }}" {{ $kepanitiaan->posisi_id_posisi == $posisi->id_posisi ? 'selected' : '' }}>{{ $posisi->nama_posisi }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                ...
                <div class="modal fade" id="kepanitiaanCreateModal" tabindex="-1" aria-labelledby="kepanitiaanCreateModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="kepanitiaanCreateModalLabel">Tambah Panitia</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
